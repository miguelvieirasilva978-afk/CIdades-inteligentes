...
/* =========================================================
   ABRIR CONTEÚDO — EVENTO DELEGADO ROBUSTO
   Funciona mesmo que os cards sejam renderizados/reorganizados.
   ========================================================= */

function initializeSingaporeDetailDelegation() {
    if (document.body.dataset.singaporeDetailDelegated === "true") return;
    document.body.dataset.singaporeDetailDelegated = "true";

    document.addEventListener("click", event => {
        const button = event.target.closest("[data-open-singapore-detail]");
        if (!button) return;

        const card = button.closest(".info-card[data-detail]");
...
function showQuizResult() {
    ...
    if (score === total) {
        title = "Perfeito!";
        message = "Você acertou todas as questões e demonstrou domínio excelente sobre cidades inteligentes e Singapura.";
        interval = "15 de 15 pontos — domínio excelente.";
    } else if (score >= 12) {
        title = "Excelente!";
        message = "Você teve um desempenho muito alto. Já domina a maior parte dos conceitos apresentados no site.";
        interval = "12 a 14 pontos — excelente domínio.";
    } else if (score >= 9) {
        title = "Muito bom!";
        message = "Você compreendeu bem os principais conceitos, mas ainda pode aprofundar alguns temas.";
        interval = "9 a 11 pontos — bom domínio, com alguns pontos para revisar.";
    } else if (score >= 6) {
        title = "Bom começo!";
        message = "Você já possui uma base sobre cidades inteligentes. Explore Singapura e Tecnologias para consolidar o conteúdo.";
        interval = "6 a 8 pontos — conhecimento em desenvolvimento.";
    } else if (score >= 3) {
        title = "Continue explorando!";
        message = "Você identificou alguns conceitos, mas vale revisar as explicações e tentar novamente.";
        interval = "3 a 5 pontos — é hora de reforçar os conceitos principais.";
    } else {
        title = "Vamos começar de novo!";
        message = "Use o site para estudar os conceitos e tente o quiz novamente. Cada tentativa ajuda a aprender.";
        interval = "0 a 2 pontos — recomendamos revisar o conteúdo antes de refazer o quiz.";
    }

    if (elements.container) elements.container.innerHTML = "";
    if (elements.resultTitle) elements.resultTitle.textContent = title;
    if (elements.finalScore) elements.finalScore.textContent = `${score}/${total}`;
    if (elements.resultMessage) elements.resultMessage.textContent = message;

    if (elements.result) {
        elements.result.hidden = false;
        elements.result.classList.remove("hidden");
        elements.result.classList.add("active", "is-visible");

        let intervalBox = elements.result.querySelector(".quiz-interval");
        if (!intervalBox) {
            intervalBox = document.createElement("div");
            intervalBox.className = "quiz-interval";
            elements.resultMessage?.insertAdjacentElement("afterend", intervalBox);
        }
        intervalBox.textContent = interval;

        const percentageEl = elements.result.querySelector("[data-quiz-percentage]");
...
function restartQuiz() {
    ...
    const resultElement = byId("quizResult");
    if (resultElement) {
        resultElement.hidden = true;
        resultElement.classList.add("hidden");
        resultElement.classList.remove("active", "is-visible");
    }
...
if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", initializeSingaporeDetailDelegation, { once: true });
} else {
    initializeSingaporeDetailDelegation();
}


function initializeSourcePriorityFiltersV4() {
    const filters = document.querySelectorAll(".source-filter");
    const cards = document.querySelectorAll(".source-card[data-category]");
    if (!filters.length || !cards.length) return;
    filters.forEach(filter => {
        if (filter.dataset.v4Initialized === "true") return;
        filter.dataset.v4Initialized = "true";
        filter.addEventListener("click", () => {
            const active = normalizeText(filter.dataset.category || "todos");
            filters.forEach(f => f.classList.toggle("active", f === filter));
            cards.forEach(card => {
                const categories = normalizeText(card.dataset.category || "").split(/\\s+/).filter(Boolean);
                const match = active === "todos" || categories.includes(active);
                card.classList.toggle("is-priority", active !== "todos" && match);
                card.classList.toggle("is-muted", active !== "todos" && !match);
            });
        });
    });
}
...
if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", initializeSourcePriorityFiltersV4, { once: true });
} else {
    initializeSourcePriorityFiltersV4();
}
